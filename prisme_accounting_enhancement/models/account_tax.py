...
class AccountTax(models.Model):
    ...
    ### PSI modification : Adding the prisme_rounding parameter to compute our "custom" rounding on the taxes
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True, prisme_rounding=None):
        ...
        for tax in taxes:
            ...
            ### PSI modification : if the prisme_rounding was given, round the taxes with it. Else, use the default ODOO rounding on taxes
            if prisme_rounding:
                tax_amount = self._prisme_round_amount(tax_amount, prisme_rounding)
                factorized_tax_amount = self._prisme_round_amount(tax_amount * sum_repartition_factor, prisme_rounding)
            else:
    ...
    def _prisme_round_amount(self, amount, factor):
        """Rounds the given amount to the given factor."""
        new_amount = builtin_round(amount / factor) * factor
        return new_amount